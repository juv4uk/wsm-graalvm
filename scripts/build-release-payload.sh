...
cp "$REPO/refs/lisp-dependency-manifest.lisp" "$OUT/share/doc/wsm-graalvm/lisp-dependency-manifest.lisp"
cp "$REPO/refs/sparse-authority-paths.txt" "$OUT/share/doc/wsm-graalvm/sparse-authority-paths.txt"
...
AUTH="$HERE/lib/wsm-graalvm/$VERSION/my-lisp"
TMP="$(mktemp --suffix=.lisp)"
trap 'rm -f "$TMP"' EXIT
cat "$AUTH/lib/canon.lisp" "$AUTH/lib/macro.lisp" "$AUTH/lib/core.lisp" "$SOURCE" > "$TMP"
"$HERE/lib/wsm-graalvm/$VERSION/native-wsm" "$TMP" "$AUTH/lib/surface/semantic-registry.lisp" "$HERE"
EOF
chmod +x "$OUT/bin/wsm"
fi
