...
[ -f "$REG" ] || {
  echo "QUOTE-READER-GATE: missing pinned registry" >&2
  exit 1
}
[ -f "$CANON" ] || {
  echo "QUOTE-READER-GATE: missing pinned canon.lisp" >&2
  exit 1
}

bash "$REPO/scripts/build.sh"

TEST_CLASSES="$REPO/test-classes-quote-reader"
rm -rf "$TEST_CLASSES"
...
"$G/bin/javac" --release 25 \
  -cp "$CP" \
  -d "$TEST_CLASSES" \
  "$REPO/tests/QuoteReaderContract.java"

"$G/bin/java" \
  -cp "$TEST_CLASSES:$CP" \
  wsm.graalvm.QuoteReaderContract "$REG" "$CANON"
