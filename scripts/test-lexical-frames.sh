...
[ -f "$REG" ] || {
  echo "LEXICAL-FRAME-GATE: missing pinned registry" >&2
  exit 1
}

bash "$REPO/scripts/build.sh"

TEST_CLASSES="$REPO/test-classes-lexical-frames"
rm -rf "$TEST_CLASSES"
...
"$G/bin/javac" --release 25 \
  -cp "$CP" \
  -d "$TEST_CLASSES" \
  "$REPO/tests/LexicalFrameContract.java"

"$G/bin/java" \
  -cp "$TEST_CLASSES:$CP" \
  wsm.graalvm.LexicalFrameContract "$REG"
