...
[ -n "$VALUE_SUMMARY" ] || {
  echo "TIER1-LEDGER FAIL: missing value summary" >&2
  exit 1
}
...
set +e
ERROR_REPORT=$("$JAVA_HOME/bin/java" -cp "$TEST_CLASSES:$CP"   wsm.graalvm.Tier1ErrorParityContract "$CORPUS" "$REGISTRY" 2>&1)
ERROR_STATUS=$?
set -e
printf '%s
' "$ERROR_REPORT"
...
[ -n "$ERROR_SUMMARY" ] || {
  echo "TIER1-LEDGER FAIL: missing ErrorKind summary" >&2
  exit 1
}
...
[ "$SKIPPED" -eq "$ERROR_FIXTURES" ] || {
  echo "TIER1-LEDGER FAIL: error fixture accounting drift $SKIPPED != $ERROR_FIXTURES" >&2; exit 1;
}
...
[ "$ERROR_TOTAL" -eq "$ERROR_FIXTURES" ] || {
  echo "TIER1-LEDGER FAIL: error total $ERROR_TOTAL != $ERROR_FIXTURES" >&2; exit 1;
}
[ "$ERROR_PASS" -ge "$ERROR_PASS_MIN" ] || {
  echo "TIER1-LEDGER REGRESSION: error pass $ERROR_PASS < $ERROR_PASS_MIN" >&2; exit 1;
}
[ "$ERROR_BLOCKED" -le "$ERROR_BLOCKED_MAX" ] || {
  echo "TIER1-LEDGER REGRESSION: error blocked $ERROR_BLOCKED > $ERROR_BLOCKED_MAX" >&2; exit 1;
}
[ "$ERROR_FAIL" -eq 0 ] || {
  echo "TIER1-LEDGER FAIL: error parity fail=$ERROR_FAIL" >&2; exit 1;
}
[ "$ERROR_STATUS" -eq 0 ] || {
  echo "TIER1-LEDGER FAIL: error parity runner exited $ERROR_STATUS" >&2; exit 1;
}

if [ "$FAIL" -gt 0 ] && [ "$VALUE_STATUS" -eq 0 ]; then
  echo "TIER1-LEDGER FAIL: value counter exited zero despite known failures" >&2
  exit 1
fi
if [ "$FAIL" -eq 0 ] && [ "$VALUE_STATUS" -ne 0 ]; then
  echo "TIER1-LEDGER FAIL: value counter failed despite zero failures" >&2
  exit 1
fi
