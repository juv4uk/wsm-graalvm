...
if [ "$ACTUAL_PIN" != "$MY_LISP_PIN" ]; then
...
[ "$TOTAL" -eq "$SELECTED" ] || {
  echo "TIER1-LEDGER FAIL: selected drift $TOTAL != $SELECTED" >&2
  exit 1
}
[ "$SKIPPED" -eq "$EXPECTED_ERROR_PASS" ] || {
  echo "TIER1-LEDGER FAIL: error count $SKIPPED != $EXPECTED_ERROR_PASS" >&2
  exit 1
}
[ "$PASS" -ge "$VALUE_PASS_MIN" ] || {
  echo "TIER1-LEDGER REGRESSION: value pass $PASS < $VALUE_PASS_MIN" >&2
  exit 1
}
[ "$FAIL" -le "$VALUE_FAIL_MAX" ] || {
  echo "TIER1-LEDGER REGRESSION: value fail $FAIL > $VALUE_FAIL_MAX" >&2
  exit 1
}

if [ "$FAIL" -gt 0 ] && [ "$STATUS" -eq 0 ]; then
  echo "TIER1-LEDGER FAIL: counter exited zero despite known failures" >&2
  exit 1
fi
if [ "$FAIL" -eq 0 ] && [ "$STATUS" -ne 0 ]; then
  echo "TIER1-LEDGER FAIL: counter failed with zero fixture failures" >&2
  exit 1
fi
