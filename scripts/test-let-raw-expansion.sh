...
TEST_CLASSES="$REPO/test-classes-let-raw-expansion"
rm -rf "$TEST_CLASSES"
...
"$G/bin/javac" --release 25 -cp "$CP" -d "$TEST_CLASSES"   "$REPO/src/test/java/wsm/graalvm/LetRawExpansionContract.java"

"$G/bin/java" -cp "$TEST_CLASSES:$CP"   wsm.graalvm.LetRawExpansionContract "$REPO"
