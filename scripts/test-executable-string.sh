...
TEST_CLASSES="$REPO/test-classes-executable-string"
...
"$G/bin/javac" --release 25 -cp "$CP" -d "$TEST_CLASSES"   "$REPO/src/test/java/wsm/graalvm/ExecutableStringContract.java"
"$G/bin/java" -cp "$TEST_CLASSES:$CP"   wsm.graalvm.ExecutableStringContract "$REGISTRY"
