...
TEST_CLASSES="$REPO/test-classes-tco-witness"
rm -rf "$TEST_CLASSES"
...
"$G/bin/javac" --release 25 -cp "$CP" -d "$TEST_CLASSES" \
  "$REPO/src/test/java/wsm/graalvm/TailCallContract.java"

"$G/bin/java" -Xss1m -cp "$TEST_CLASSES:$CP" \
  wsm.graalvm.TailCallContract "$REPO"
