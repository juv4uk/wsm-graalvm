...
"$G/bin/javac" --release 25 -cp "$CP" -d "$TEST_CLASSES"   "$REPO/src/test/java/wsm/graalvm/ReaderStringContract.java"

"$G/bin/java" -cp "$TEST_CLASSES:$CP" wsm.graalvm.ReaderStringContract
