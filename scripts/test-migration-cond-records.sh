...
TEST_CLASSES="$REPO/test-classes-migration-cond"
rm -rf "$TEST_CLASSES"
...
"$G/bin/javac" --release 25 -cp "$CP" -d "$TEST_CLASSES"   "$REPO/src/test/java/wsm/graalvm/MigrationCondRecordContract.java"

"$G/bin/java" -cp "$TEST_CLASSES:$CP"   wsm.graalvm.MigrationCondRecordContract "$REGISTRY"
