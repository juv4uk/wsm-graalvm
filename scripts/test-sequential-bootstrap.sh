...
TEST_CLASSES="$REPO/test-classes-sequential-bootstrap"
...
"$G/bin/javac" --release 25 -cp "$CP" -d "$TEST_CLASSES"   "$REPO/src/test/java/wsm/graalvm/SequentialBootstrapContract.java"
"$G/bin/java" -cp "$TEST_CLASSES:$CP" -Dtruffle.class.path.append="$REPO/classes"   wsm.graalvm.SequentialBootstrapContract "$REGISTRY"
