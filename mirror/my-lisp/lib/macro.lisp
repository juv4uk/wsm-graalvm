  ...
  (eval
    (cons (string->symbol "0010")
      (cons (quote args)
        (quote
          ((?:
             ((.? args) (structural-kind empty-list)
              (make-macro))
             ((.? (cdr args)) (structural-kind empty-list)
              (make-macro))
             (t t
...
; COND migration provenance (#122, 2026-09-18):
; - (.? args) and (.? (cdr args)) are structural-kind queries over proper lists;
;   canonical dispatch matches the explicit empty-list result instead of truthiness.
; - fallback t is matched explicitly as the symbol t; it is not a universal truth
;   coercion and is used only as the final authored clause in this bootstrap form.
; - No Java/evaluator semantic shortcut is required by this source.
